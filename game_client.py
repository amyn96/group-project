...
lbl_welcome.pack()
serverLineLabel = tk.Label(MsgTopFrame, text="______________________________________________________________")
...
lineLabel = tk.Label(middleFrame, text="______________________________________________________________").pack()
...
lineLabel = tk.Label(finalFrame, text="______________________________________________________________").pack()
...
# DISCONNECT THE PLAYER AND CLOSE THE GAME
def disconnect():
    global dis, result
    dis = Toplevel(win)
    dis.title("disconnect")
    dis.geometry("400x300")
    dis.config(bg="white")
...
# POP UP MESSAGE FOR THE GAME RESULT
def popMsg(round_result):
    ...
    if round_result == "WIN":
        iwin1 = Image.open("win.png")
        iwin2 = iwin1.resize((190, 150), Image.ANTIALIAS)
        result = ImageTk.PhotoImage(iwin2)
...
def serverMsg(sck, m):
    ...
    while True:
        ...
        if fromS.startswith("welcome"):
            print("yes")
            if fromS == "welcome1":
                lbl_welcome["text"] = "WELCOME " + playerName + "! Waiting for the other player"
            elif fromS == "welcome2":
                lbl_welcome["text"] = "WELCOME " + playerName + "! The Game will start soon"
